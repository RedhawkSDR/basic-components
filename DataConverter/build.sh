...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e DataConverter.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/DataConverter-1.0.0
        tar czf ${tmpdir}/DataConverter-1.0.0.tar.gz --exclude=".svn" -C ${tmpdir} DataConverter-1.0.0
        rpmbuild -ta ${tmpdir}/DataConverter-1.0.0.tar.gz
        rm -rf $tmpdir
    else
