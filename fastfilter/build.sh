...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e fastfilter.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/fastfilter-1.0.0
        tar czf ${tmpdir}/fastfilter-1.0.0.tar.gz --exclude=".svn" -C ${tmpdir} fastfilter-1.0.0
        rpmbuild -ta ${tmpdir}/fastfilter-1.0.0.tar.gz
        rm -rf $tmpdir
    else
