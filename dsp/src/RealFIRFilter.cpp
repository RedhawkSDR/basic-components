...
//
// Description:
//   This file contains the RealFIRFilter class implementation.  Filtering code was
...
RealFIRFilter::RealFIRFilter(
    const RealArray &input, RealArray &output,
    Real *coef, size_t length) :
    vIn(input),
    vOut(output),
    _filtCoeff(coef, length),
    _z(Real(0), _filtCoeff.size())
{
    // Validate parameters
    if( coef == NULL )
...
//
// Description:
//   RealFIRFilter's constructor.  It implements a FIR filter with coefficients
...
RealFIRFilter::RealFIRFilter(const RealArray &input, RealArray &output,
...
RealFIRFilter::~RealFIRFilter(void)
{
    //
    // Hook
    //
...
void RealFIRFilter::run(void)
{
    ...
    const Real *endIn = &vIn[vIn.size() - 1];
    const Real *ptrIn = &vIn[lenCoef2 - 1];
...
Real RealFIRFilter::run(const Real &input)
{
    // Start at beginning of history
    Real *ptrHist, *ptrHist1;
...
size_t RealFIRFilter::size(void)
{
...
void RealFIRFilter::kaiser (RealArray &w, Real beta)
{
...
RealArray RealFIRFilter::getFilterCoefficients(void)
{
    return _filtCoeff;
}
