...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e DataReader.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/DataReader-1.0.0
        tar czf ${tmpdir}/DataReader-1.0.0.tar.gz --exclude=".svn" -C ${tmpdir} DataReader-1.0.0
        rpmbuild -ta ${tmpdir}/DataReader-1.0.0.tar.gz
        rm -rf $tmpdir
    else
