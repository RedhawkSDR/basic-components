...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e medianfilter.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/medianfilter-1.0.0
        tar czf ${tmpdir}/medianfilter-1.0.0.tar.gz --exclude=".svn" -C ${tmpdir} medianfilter-1.0.0
        rpmbuild -ta ${tmpdir}/medianfilter-1.0.0.tar.gz
        rm -rf $tmpdir
    else
